...
export default function App() {
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      "dinali_ticket_database",
      JSON.stringify(ticketDatabase)
    );
  ...
  const handleGenerateTicket = (e: any) => {
    ...
    if (!name && !isAdmin) return; // Name required for public, but admin might just want "Guest 1"
    const guestName = name || "VIP Guest";

    if (isAdmin) {
      // Admin bypasses the approval process
  ...
  const handleAdminApprove = async () => {
    ...
    const payload = {
      ...
      quantity: quantity,
      ticketId: uniqueId,
      ticketNumber: `${ticketsSold + 1}${
        quantity > 1 ? ` - ${newTicketNumber}` : ""
      }`,
    ...
    try {
      ...
      // Only attempt fetch if the URL has been updated
      if (GOOGLE_API_URL !== "YOUR_GOOGLE_SCRIPT_URL_HERE") {
        await fetch(GOOGLE_API_URL, {
          method: "POST",
          // Using text/plain avoids CORS preflight issues and CodeSandbox interceptor crashes
          headers: {
      ...
      // Generate UI Ticket after small delay for effect
      setTimeout(() => {
        generateFinalTicket(name, email, quantity, uniqueId);
      }, 800);
    } catch (error) {
      console.error("Failed to connect to backend:", error);
      // Fallback to generate ticket locally even if network fails
      setTimeout(() => {
        generateFinalTicket(name, email, quantity, uniqueId);
      }, 800);
    }
  ...
  const generateFinalTicket = (
    ...
    const formattedNumber = newTicketNumber.toString().padStart(3, "0");
    // Use the predefined ID from the backend call, or make a new one if bypassing
    const uniqueId =
      predefinedId ||
      `DINALI-26-${formattedNumber}-${Math.random()
        .toString(36)
        .substr(2, 5)
        .toUpperCase()}`;
  ...
  const progressPercentage = (ticketsSold / MAX_TICKETS) * 100;
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 py-8 flex flex-col items-center min-h-screen">
        ...
        <div className="text-center w-full flex flex-col items-center relative z-20">
          ...
          <div className="relative w-full flex flex-col items-center justify-center z-30 mb-8 md:mb-16">
            {/* Massive angelic backlight behind Title */}
        ...
        <div className="w-full flex justify-center mt-8 md:mt-12 relative z-30">
          {isAdmin && !userTicket ? (
              ...
              <div className="relative w-full bg-[#0a120a]/90 border border-green-900/40 backdrop-blur-2xl rounded-3xl p-8 md:p-10 shadow-2xl">
                <div className="flex justify-between items-center mb-8 border-b border-green-900/50 pb-6">
                  <div>
                    ...
                    <p className="text-green-500 text-xs uppercase tracking-[0.2em]">
                      soulsoundsbydinali@gmail.com
                    </p>
                  </div>
                  <button
                    onClick={() => setIsAdmin(false)}
                    className="text-xs uppercase tracking-widest text-gray-400 hover:text-white border border-gray-600 px-4 py-2 rounded-full"
                  >
                    Exit Admin
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                  ...
                  <div className="lg:col-span-1 bg-black/40 border border-green-900/20 p-6 rounded-2xl">
                    <h3 className="text-sm font-medium uppercase tracking-widest text-green-400 mb-6">
                      Direct Ticket Generator
                    </h3>
                    <p className="text-xs text-gray-400 mb-6">
                      Generate tickets instantly for your guest list without
                      approval. You can download the generated ticket to send
                      via WhatsApp.
                    </p>
                    ...
                    <form onSubmit={handleGenerateTicket} className="space-y-4">
                      ...
                      <button
                        type="submit"
                        className="w-full bg-green-600 hover:bg-green-500 text-black font-bold uppercase tracking-widest text-xs py-3 rounded mt-4 transition-colors"
                      >
                  ...
                  <div className="lg:col-span-2">
                    <h3 className="text-sm font-medium uppercase tracking-widest text-green-400 mb-4">
                      Ticket Database (Local Demo)
                    </h3>
                    ...
                    <p className="text-[10px] text-gray-600 mt-4 leading-relaxed">
                      * In a production environment, this dashboard connects
                      directly to Google Sheets via Apps Script, allowing you to
                      track emails, approve requests, and automate
            ...
            <div className="w-full max-w-lg relative group">
              ...
              <div className="relative w-full bg-[#1a0205]/80 border border-red-900/40 backdrop-blur-2xl rounded-3xl p-8 md:p-10 shadow-2xl">
                <div className="flex justify-between items-end mb-8 border-b border-red-900/50 pb-6">
                  <div>
                    <h2 className="text-3xl font-light tracking-wide mb-1 text-white">
                      Reserve
                    </h2>
                ...
                {ticketsRemaining > 0 ? (
                  <form onSubmit={handleGenerateTicket} className="space-y-6">
                    ...
                    <div className="pt-4 flex items-center justify-between border-b border-red-900/50 pb-2">
                      <span className="text-gray-300 font-light tracking-wide px-2">
                        Passes
                      </span>
                      <select
                        value={quantity}
                      ...
                      >
                        {[...Array(Math.min(10, ticketsRemaining))].map(
                          (_, i) => (
                            <option
                              key={i + 1}
                              value={i + 1}
                    ...
                    <div className="pt-8">
                      ...
                      >
                        ...
                        {requestStatus === "submitting" ? (
                          ...
                          <>
                            <span>
                              Secure Passes • ${quantity * TICKET_PRICE}
                            </span>
            ...
            <div className="w-full max-w-md bg-[#1a0205]/80 border border-red-900/40 backdrop-blur-2xl rounded-3xl p-10 shadow-2xl text-center animate-fade-in">
              ...
              <h2 className="text-xl font-light tracking-widest uppercase mb-4 text-white">
                Request Lodged
              </h2>
              <p className="text-gray-400 font-light text-sm leading-relaxed mb-8">
                Verification sent for <span className="text-white">{name}</span>
                . Awaiting final clearance for {quantity} pass
                {quantity > 1 ? "es" : ""}.
              </p>

              <div className="mt-8 pt-8 border-t border-white/5 relative">
                <p className="text-[10px] text-gray-600 tracking-[0.3em] uppercase mb-4">
                  Simulate Clearance
                </p>
                <button
            ...
            <div className="w-full flex flex-col items-center animate-fade-in-up mt-[-40px]">
              <p className="text-xs text-green-400 tracking-[0.3em] uppercase mb-12 animate-pulse drop-shadow-md">
                ...
                >
                    ...
                    <div className="relative z-10 flex flex-col h-full justify-between min-h-[350px]">
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <h2
                            className="text-xs md:text-sm uppercase tracking-[0.5em] text-gray-400 mb-4"
                            style={{ fontFamily: "'Montserrat', sans-serif" }}
                          >
                            A Musical Journey
                          </h2>
                        ...
                        <div className="flex flex-col items-end space-y-4">
                          <div className="w-16 h-16 rounded-full border border-white/20 bg-gradient-to-br from-yellow-300/20 via-yellow-600/20 to-orange-300/20 flex items-center justify-center backdrop-blur-md">
                            <span className="text-[8px] tracking-widest text-white/50 text-center uppercase leading-tight">
                              Official
                              <br />
                              Seal
                            </span>
                          ...
                          <div className="w-20 h-28 md:w-28 md:h-36 rounded-md overflow-hidden border border-[#D4AF37]/40 shadow-[0_10px_20px_rgba(0,0,0,0.6)] relative">
                            {/* Subtle dark gradient overlay at bottom to blend into the ticket */}
                            ...
                            <img
                              ...
                              onError={(e: any) => {
                                ...
                                e.target.src =
                                  "https://images.unsplash.com/photo-1544005313-94ddf0286df2?auto=format&fit=crop&w=400&q=80"; // Fallback to placeholder if local file isn't found
                              }}
                            />
                          </div>
                        </div>
                  ...
                  <div className="relative w-full md:w-80 bg-[#170104] border-t border-red-900/40 md:border-t-0 md:border-l p-8 flex flex-col items-center justify-between">
                    ...
                    <div className="w-full">
                      <p className="text-[9px] text-gray-400 uppercase tracking-[0.3em] mb-1">
                        Guest Name
                      </p>
                      <p className="text-lg font-light tracking-wider uppercase border-b border-red-900/40 pb-4 mb-4 text-white">
                        {userTicket.name}
                      </p>

                      <div className="flex justify-between items-end mb-8">
                        <div>
                          <p className="text-[9px] text-gray-500 uppercase tracking-[0.3em] mb-1">
                            Section
                          </p>
                          <p className="text-xl font-light text-[#D4AF37] tracking-wider">
                            VIP
                          </p>
                        </div>
                        <div className="text-right">
                      ...
                      {[...Array(30)].map((_, i) => (
                        <div
                          ...
                          style={{
                            ...
                            marginRight: Math.random() * 3 + 1 + "px",
                          }}
                        ></div>
                      ))}
                    </div>
        ...
        <button
          ...
          className={`p-3 rounded-full backdrop-blur-md transition-all ${
            isAdmin
              ...
              : "bg-black/40 text-gray-600 border border-white/10 hover:text-white"
          }`}
